...
七、模型异常/沟通处理方案
【基牙咬合空间不足】
- "XX医生您好，制作XXX患者义齿时，发现模型基牙咬合空间不足，达不到对应材料的最小厚度要求，无法直接加工。想跟您确认：是在模型上修整基牙/对颌牙并做好标记，还是麻烦您重新备牙取模？"
...
只输出润色后的完整回复文案，不要任何解释说明。`;
  ...
  try {
    ...
    if (hasImages) {
      if (hasText) {
        // 有图片+有文字：结合图片信息和文字进行润色
        userContent = `【截图识别】请仔细分析这些截图，提取：患者姓名、医生姓氏、问题描述、沟通场景等有用信息。不要提取订单号。\n\n`;
        userContent += `【补充文字】：${text}\n\n`;
        userContent += `请根据截图识别的信息和补充文字，生成一条专业、礼貌的客服回复。`;
      } else {
        // 只有图片（无文字）：自动匹配话术库
        userContent = `【纯图片识别任务】
请仔细分析这些截图，识别客户的问题场景。
...
请完成以下两个步骤：
1. 【识别场景】：从截图中识别这是什么情况（模型问题/交期咨询/物料缺失/质量问题等）
2. 【匹配话术】：从下方话术库中找到最匹配的一条，替换XX为从图片中提取的具体信息（患者姓名用XX代替医生姓氏），直接输出话术原文
